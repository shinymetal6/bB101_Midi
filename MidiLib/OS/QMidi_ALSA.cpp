...
void QMidiInternal::MidiInReceiveThread::run()
{
	...
	while (!isInterruptionRequested() && fMidiIn->isConnected()) {
		...
		{
            sysex_msg = QByteArray(reinterpret_cast<const char*>(ev->data.ext.ptr), ev->data.ext.len);
            //qDebug() << "ALSA :  " << sysex_msg;
            //emit(fMidiIn->midiSysExEvent(sysex_msg));
            //qDebug() << "Emitted";
